...
def scaled_dot_product_two_loop_single(
    ...
    # Replace "pass" statement with your code
    K,M = query.shape
    attn = torch.zeros((K,K), device=query.device)
    out = torch.zeros_like(query)
    for i in range(K):
        for j in range(K):
            attn[i][j] = torch.dot(query[i],key[j]) / (M ** 0.5)
...
def scaled_dot_product_two_loop_batch(
    ...
    ###############################################################################
    # Replace "pass" statement with your code
    attn = torch.zeros((N, K, K), device=query.device)
    out = torch.zeros_like(query)
    for i in range(N):
        attn[i] = torch.mm(query[i], key[i].transpose(0, 1)) / (M ** 0.5)
...
def scaled_dot_product_no_loop_batch(
    ...
    ###############################################################################
    # Replace "pass" statement with your code
    weights_softmax = torch.bmm(query, key.transpose(1, 2)) / (M ** 0.5)
    ...
    if mask is not None:
        ...
        ##########################################################################
        # Replace "pass" statement with your code
        weights_softmax = weights_softmax.masked_fill(mask, -1e9)
    # Replace "pass" statement with your code
    weights_softmax = F.softmax(weights_softmax, dim=-1)
...
class MultiHeadAttention(nn.Module):
    def __init__(self, num_heads: int, dim_in: int, dim_out: int):
        ...
        self.mlp = nn.Linear(num_heads * dim_out, dim_in)
        nn.init.uniform_(self.mlp.weight, -c, c)
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
    ...
    def forward(
        ...
        ##########################################################################
        # Replace "pass" statement with your code
        y = torch.cat([head.forward(query, key, value, mask) for head in self.heads], dim=-1)
        y = self.mlp(y)
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
...
class LayerNormalization(nn.Module):
    ...
    def forward(self, x: Tensor):
        ...
        y = x_normalized * self.scale + self.shift
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
        return y
...
class EncoderBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        n1 = self.ln1(x + attn_out)
        d1 = self.dropout(n1)
        ff_out = self.ff(d1)
        n2 = self.ln2(d1 + ff_out)
        y = self.dropout(n2)
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
...
def get_subsequent_mask(seq):
    ...
    N = seq.shape[0]
    K = seq.shape[1]
    mask = torch.zeros((N, K, K), device=seq.device).bool()
    subsequent_mask = torch.triu(torch.ones((K, K), device=seq.device), diagonal=1).bool()
...
class DecoderBlock(nn.Module):
    ...
    def forward(
        ...
        ##########################################################################
        # Replace "pass" statement with your code
        attn_out = self.attention_self(dec_inp, dec_inp, dec_inp, mask)
        n1 = self.norm1(dec_inp + attn_out)
        d1 = self.dropout(n1)
        cross_attn_out = self.attention_cross(d1,enc_inp,enc_inp)
        n2 = self.norm2(d1 + cross_attn_out)
        d2 = self.dropout(n2)
        ff_out = self.feed_forward(d2)
        n3 = self.norm3(d2 + ff_out)
        y = self.dropout(n3)
        print(y)
        ##########################################################################
        #               END OF YOUR CODE                                         #
...
def position_encoding_simple(K: int, M: int) -> Tensor:
    ...
    y = positions.unsqueeze(1).expand(-1, M).unsqueeze(0)
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
    return y
...
def position_encoding_sinusoid(K: int, M: int) -> Tensor:
    ...
    for pos in range(K):
        for i in range(0, M, 2):
            pe[pos, i] = torch.sin(torch.tensor(pos))
            if i + 1 < M:
                pe[pos, i + 1] = torch.cos(torch.tensor(pos))
    y = pe.unsqueeze(0)
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
