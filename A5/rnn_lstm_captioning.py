...
def rnn_backward(dh, cache):
    ...
    dx = []
    N,T,H = dh.shape
    tmp_h,dWx,dWh,db = 0,0,0,0
    for i in reversed(range(T)):
        dnext_h = dh[:, i, :] + tmp_h
        dx_i, dh_i, dWx_i, dWh_i, db_i = rnn_step_backward(dnext_h, cache[i])
        dx.insert(0,dx_i)
        dWx += dWx_i
        dWh += dWh_i
        db += db_i
...
class CaptioningRNN(nn.Module):
    ...
    def forward(self, images, captions):
        ...
        if self.cell_type == "attn":
            ...
            h0 = self.feature_proj(features.mean(dim=(2,3)))
        word_embedding = self.word_embedding(captions_in)
        if self.cell_type == "attn":
            hiddens = self.rnn(word_embedding, A)
        else:
            hiddens = self.rnn(word_embedding, h0)
    ...
    def sample(self, images, max_length=15):
        ...
        current_word = captions[:, 0].view(N,1)
        if self.cell_type == "attn":
            h = A.mean(dim=(2,3))
            c = h
        for i in range(max_length):
            embeded_word = self.word_embedding(current_word).squeeze(1)
            if self.cell_type == "rnn":
                h_next = self.rnn.step_forward(embeded_word, h)
                scores = self.output_proj(h_next)
                current_word = torch.argmax(scores, dim=1)
                captions[:, i] = current_word
                current_word = current_word.view(N,1)
                h = h_next
            elif self.cell_type == "lstm":
                h_next,c_next = self.rnn.step_forward(embeded_word, h,c)
                scores = self.output_proj(h_next)
                current_word = torch.argmax(scores, dim=1)
                captions[:, i] = current_word
                current_word = current_word.view(N,1)
                ...
                h_next,c_next = self.rnn.step_forward(embeded_word, h,c,attn)
                scores = self.output_proj(h_next)
                current_word = torch.argmax(scores, dim=1)
                captions[:, i] = current_word
                current_word = current_word.view(N,1)
...
class LSTM(nn.Module):
    ...
    def forward(self, x: torch.Tensor, h0: torch.Tensor) -> torch.Tensor:
        ...
        c = torch.zeros((N, T, H),device=x.device)
        h = h0
        c = c0
        for i in range(T):
...
def dot_product_attention(prev_h, A):
    ...
    # Replace "pass" statement with your code
    A_bar = A.view(N, H, -1)
    attn_weights = (torch.bmm(prev_h.view(N, 1, H), A_bar).squeeze(1) / (H ** 0.5)).softmax(dim=1)
    attn = torch.bmm(A_bar, attn_weights.unsqueeze(2)).squeeze(2)
...
class AttentionLSTM(nn.Module):
    ...
    def forward(self, x: torch.Tensor, A: torch.Tensor):
        ...
        # Replace "pass" statement with your code
        N, T, D = x.shape
        hn = torch.zeros((N, T, h0.shape[1]), device=x.device)
