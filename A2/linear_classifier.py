...
def svm_loss_vectorized(
    ...
    scores = (scores.t() - correct_class_scores.view(len(y)) + 1).t()
    scores[range(X.shape[0]),y] = 0
    scores[scores < 0] = 0
    loss = scores.sum()/X.shape[0] + reg * torch.sum(W * W)
    #############################################################################
    #                             END OF YOUR CODE                              #
    #############################################################################
    ...
    # Replace "pass" statement with your code
    scores[scores > 0] = 1
    num_of_incorrect = scores.sum(dim=1)
    scores[range(X.shape[0]),y] = -num_of_incorrect
    dW += 2 * reg * W
    dW += scores.t().mm(X).t()/X.shape[0]
    #############################################################################
    #                             END OF YOUR CODE                              #
    #############################################################################
...
def softmax_loss_naive(
    ...
    num_classes = W.shape[1]
    for i in range(num_train):
        scores_i = scores[i] - scores[i].max()
        scores_i = torch.exp(scores_i)
        scores_i = scores_i / torch.sum(scores_i)
        loss_i = -torch.log(scores_i[y[i]])
        loss += loss_i
        for j in range(num_classes):
            if j == y[i]:
...
def softmax_loss_vectorized(
    ...
    scores = torch.exp(scores - max_scores)
    scores = scores / torch.sum(scores,dim = 1, keepdim = True)
    loss = torch.sum(-torch.log(scores[range(num_train),y]))
    loss /= num_train
    loss += reg * torch.sum(W * W)
