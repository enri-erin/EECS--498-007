...
def nn_forward_backward(
    ...
    correct_log_probs = -torch.log(probs[range(num_train), y])
    data_loss = correct_log_probs.sum() / num_train
    ...
    dh1_relu = dh1 * (h1 > 0).float()

    dW1 = X.t().mm(dh1_relu)
    dW1 += 2 * reg * W1
    grads['W1'] = dW1

    db1 = dh1_relu.sum(dim=0)
    grads['b1'] = db1
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def nn_predict(
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    scores = X.mm(params['W1']) + params['b1']
    scores = torch.relu(scores)
    scores = scores.mm(params['W2']) + params['b2']
        ...
        for hs in hidden_sizes:
            for reg in regularization_strengths:
                for decay in learning_rate_decays:
                    ...
                    stat = net.train(data_dict['X_train'], data_dict['y_train'], data_dict['X_val'], data_dict['y_val'],learning_rate = lr, learning_rate_decay = decay,reg = reg,num_iters=1000, batch_size=1000, verbose=False)
                    val_acc = 100 * (net.predict(data_dict['X_val']) == data_dict['y_val']).double().mean().item()
                    print(f"lr: {lr}, hs: {hs}, reg: {reg}, decay: {decay}, val_acc: {val_acc}")
