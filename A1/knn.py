...
def predict_labels(dists: torch.Tensor, y_train: torch.Tensor, k: int = 1):
    ...
    # Replace "pass" statement with your code
    indices = None
    values,indices = torch.topk(dists,k=k,largest=False,dim=0)
    indices = y_train[indices].t()
    indices,sort_indices = torch.sort(indices,dim=1)
    for i in range(num_test):
        unique_values,counts = torch.unique(indices[i],return_counts=True)
        y_pred[i] = unique_values[counts.argmax()]
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return y_pred
...
def knn_cross_validate(
    ...
    for i in range(len(k_choices)):
        ...
        for j in range(num_folds):
            tmp_x_validation_fold = x_train_folds[j]
            tmp_y_validation_fold = y_train_folds[j]
            tmp1 = list(x_train_folds[:j]) + list(x_train_folds[j+1:])
            tmp2 = list(y_train_folds[:j]) + list(y_train_folds[j+1:])
            tmp1 = tuple(tmp1)
            tmp2 = tuple(tmp2)
            tmp_x_train_folds = torch.cat(tmp1)
            tmp_y_train_folds = torch.cat(tmp2)
            k_to_accuracies[k_choices[i]].append(KnnClassifier(tmp_x_train_folds,tmp_y_train_folds)
                .check_accuracy(tmp_x_validation_fold,tmp_y_validation_fold,k=k_choices[i],quiet=True))
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
...
def knn_get_best_k(k_to_accuracies: Dict[int, List]):
    ...
    for key in k_to_accuracies:
        accuracy = torch.tensor(k_to_accuracies[key])
        mean_accuracy = torch.mean(accuracy)
        if mean_accuracy > max:
            max = mean_accuracy
            best_k = key
    ##########################################################################
    #                           END OF YOUR CODE                             #
