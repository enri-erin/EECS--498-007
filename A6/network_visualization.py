...
def compute_saliency_maps(X, y, model):
    ...
    scores = model(X)
    correct_scores = scores[torch.arange(X.size(0)), y]
...
def make_adversarial_attack(X, target_y, model, max_iter=100, verbose=True):
    ...
    for i in range(max_iter):
        ...
        scores = model(X_adv)
        correct_scores = scores[torch.arange(X_adv.size(0)), target_y]
        loss = torch.sum(correct_scores)
        loss.backward()
        X_adv.data += learning_rate * X_adv.grad.data / X_adv.grad.data.norm()
        X_adv.grad.data.zero_()
        if verbose:
            print("Generating adversarial attack...")
            print(f"Iteration {i + 1}/{max_iter}")
            print(f"Score: {scores[0, target_y].item()}")
            print(f"Loss: {loss.item()}")
            print(f"Adversarial image: {X_adv[0].data}")
        # Check if the model is fooled
        if max(scores[0]) == scores[0, target_y].item():
            print("Adversarial attack generated successfully!")
            break
    ##############################################################################
    #                             END OF YOUR CODE                               #
    ##############################################################################
...
def class_visualization_step(img, target_y, model, **kwargs):
    ...
    img.requires_grad_()
    scores = model(img)
    correct_scores = scores[torch.arange(img.size(0)), target_y]
    loss = torch.sum(correct_scores) - l2_reg * torch.sum(img ** 2)
    loss.backward()
